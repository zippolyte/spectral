  ...
  describe('reports unreferenced definitions', () => {
    test('when analyzing an in-memory document', async () => {
      ...
      const remoteFsRefeferencePath = path.join(
        __dirname,
        '../../__tests__/__fixtures__/unusedShared/unusedDefinition.definition.json#/definitions/ExternalFs',
      );
    ...
    test('when analyzing a directly self-referencing document from the filesystem', async () => {
      const fixturePath = path.join(
        __dirname,
        '../../__tests__/__fixtures__/unusedShared/unusedDefinition.remoteLocal.json',
      );
    ...
    test('when analyzing an indirectly self-referencing document from the filesystem', async () => {
      const fixturePath = path.join(
        __dirname,
        '../../__tests__/__fixtures__/unusedShared/unusedDefinition.indirect.1.json',
      );
      ...
      expect(results).toEqual([
        {
          ...
          severity: DiagnosticSeverity.Warning,
          source: expect.stringMatching('/__tests__/__fixtures__/unusedShared/unusedDefinition.indirect.1.json$'),
        },
      ]);
    });
  });
